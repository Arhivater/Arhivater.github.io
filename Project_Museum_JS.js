...
for(let i = 1; i <= 4 ; i++) {
	...
	section.id=("Paintings_section_"+i);
...
var temp = 0;
//добавляет секцию в раннее осзданное место
for(let j = 1; j <= 4 ; j++) {
	for(let i = 1; i <= 3 ; i++) {
		var Painting_img = document.getElementById("Painting"+'_'+j+'_'+i);
		Painting_img.insertAdjacentHTML("beforeend", "<img class='img_Paintings' src='"+ Paintings_img[temp] +"' alt='Painting_Image'>");
		temp = temp + 1;
	}
}
...
Painting_content.insertAdjacentHTML("beforeend", "<div id='Painting_content"+'_'+j+'_'+i+"' class='w3-container w3-white'>");
var Content = document.getElementById("Painting_content"+'_'+j+'_'+i);
Content.insertAdjacentHTML("beforeend", "<div id='Content_media"+'_'+j+'_'+i+"' class='media'>");
}
}

for(let j = 1; j <= 4 ; j++) {
for(let i = 1; i <= 3 ; i++) {
var Media = document.getElementById("Content_media"+'_'+j+'_'+i);
Media.insertAdjacentHTML("beforebegin", "<div class='media-left' style='padding-top: 35px;'>");
}
}
